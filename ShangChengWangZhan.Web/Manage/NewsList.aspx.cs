...
namespace Shopping.Web.Manage
{
    public partial class NewsList : ManagePage
    {
        ...
        {
            GetCondition();
            datagrid.VirtualItemCount = NewsBLL.GetNoticeCount(condition.ToString());
            datagrid.DataSource = NewsBLL.GetNoticeList(datagrid.PageSize, datagrid.CurrentPageIndex + 1, condition.ToString());
            datagrid.DataBind();
        }
        /// <summary>
        /// 获取条件
        ...
        {
            bool flag = NewsBLL.DeleteNotice(Id);
            if (flag)
            {
                ShowOK("删除成功!");
            }
