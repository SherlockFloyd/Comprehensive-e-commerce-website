...
KE.setting = {
	...
	minChangeSize : 5,
	items : [
		'date', 'time', 'undo', 'redo', 'print', 'cut', 'copy', 'paste',
		'plainpaste', 'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright',
		'justifyfull', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript',
		'superscript', '|', 'selectall', '-',
		'title', 'fontname', 'fontsize', '|', 'textcolor', 'bgcolor', 'bold',
		'italic', 'underline', 'nextpage', 'strikethrough', 'removeformat', '|', 'image', 'multiimage',
		'flash', 'media', 'advtable', 'hr', 'emoticons', 'link', 'unlink', '|', 'about', '|', 'source', 'fullscreen', 'preview'
	],
	colorTable : [
...
KE.util = {
	...
	drag : function(id, mousedownObj, moveObj, func) {
		...
		mousedownObj.onmousedown = function(e) {
			...
			var seleShoppingistener = function() { return false; };
			function upListener(e) {
				...
				KE.event.remove(document, 'selectstart', seleShoppingistener);
				KE.event.stop(e);
				return false;
			}
			...
			KE.event.add(document, 'selectstart', seleShoppingistener);
...
KE.plugin['image'] = {
	...
	exec : function(id) {
		...
		var href = KE.$('linkUrl', dialogDoc).value;
		...
		if (type == 2) {
			...
			var onloadFunc = function() {
				...
				if (typeof data === 'object' && 'error' in data) {
					if (data.error === 0) {
						self.insert(id, data.url, title, width, height, 0, align, href, target);
					} else {
						alert(data.message);
						return false;
					}
			...
			var url = KE.$('url', dialogDoc).value;
			this.insert(id, url, title, width, height, 0, align, href, target);
		}
	},
	insert : function(id, url, title, width, height, border, align, href, target) {
		...
		html += 'border="' + border + '" />';
		if(href){
			var temp = '<a href="' + href + '"';
			if (target) temp += ' target="' + target + '"';
			temp += '>' + html + '</a>';
			html=temp;
			}
		html +='<br />';
