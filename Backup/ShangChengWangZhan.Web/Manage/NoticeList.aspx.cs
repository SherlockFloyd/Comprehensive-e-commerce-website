...
{
    public partial class NoticeList : ManagePage
    {
        ...
        {
            GetCondition();
            datagrid.VirtualItemCount = NoticeBLL.GetNoticeCount(condition.ToString());
            datagrid.DataSource = NoticeBLL.GetNoticeList(datagrid.PageSize, datagrid.CurrentPageIndex + 1, condition.ToString());
            datagrid.DataBind();
        }
        /// <summary>
        /// 获取条件
        ...
        private string GetCondition()
        {
            ...
            if (keyword != "")
            {
                ...
                condition.AppendFormat(" Title like '%{0}%'", keyword);
            }
        ...
        {
            bool flag = NoticeBLL.DeleteNotice(Id);
            if (flag)
            {
                ShowOK("删除成功!");
            }
