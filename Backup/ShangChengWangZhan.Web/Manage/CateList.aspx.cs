...
{
    public partial class CateList : ManagePage
    {
        ...
        {
            GetCondition();
            datagrid.VirtualItemCount = CateBLL.GetCateCount(condition.ToString());
            datagrid.DataSource = CateBLL.GetCateList(datagrid.PageSize, datagrid.CurrentPageIndex + 1, condition.ToString());
            datagrid.DataBind();
        }
        /// <summary>
        /// 获取条件
        ...
        private void Delete(int Id)
        {
            ...
            bool flag = CateBLL.DeleteCate(Id);
            if (flag)
            {
                ShowOK("删除成功!");
            }
